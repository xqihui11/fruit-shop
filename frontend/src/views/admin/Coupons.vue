<template>
  <div class="coupons-page">
    <el-card class="coupons-card" shadow="never">
      <template #header>
        <div class="card-header">
          <div class="card-title-wrap">
            ...
            <div class="card-subtitle">创建和管理平台优惠券，提升用户购买转化</div>
          </div>
      ...
      <div class="toolbar">
        ...
        <div class="filter-section">
          ...
          <el-select v-model="publisherFilter" placeholder="发放主体" clearable @change="handleFilter" style="width: 120px;">
            <el-option label="全部" value="" />
            <el-option label="平台优惠券" :value="null" />
            <el-option label="商家优惠券" :value="'merchant'" />
          </el-select>
          <el-select 
            v-model="merchantFilter" 
            placeholder="选择商家（可搜索）" 
            clearable 
            filterable
            :filter-method="filterMerchants"
            @change="handleFilter" 
            style="width: 220px;"
          >
            ...
            >
              <div class="merchant-option">
                <span class="merchant-name">{{ merchant.shopName || merchant.username }}</span>
                <span v-if="merchant.shopName && merchant.username !== merchant.shopName" class="merchant-username">
                  ({{ merchant.username }})
                </span>
              </div>
            </el-option>
      ...
      <div v-if="selectedCoupons.length > 0" class="batch-actions">
        <span class="selected-count">已选择 {{ selectedCoupons.length }} 项</span>
        <el-button size="small" @click="batchToggleStatus">批量{{ selectedCoupons[0]?.status === 1 ? '下架' : '上架' }}</el-button>
    ...
    <!-- 编辑对话框（保留作为备用） -->
    <el-dialog v-model="dialogVisible" :title="editingCoupon ? '编辑优惠券' : '添加优惠券'" width="600px" style="display: none;">
      <el-form :model="form" :rules="rules" ref="formRef" label-width="120px">
        <el-form-item label="优惠券名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入优惠券名称" />
        </el-form-item>
...
const merchants = ref([])
const filteredMerchantList = ref([])
...
const filteredCoupons = computed(() => {
  ...
  if (publisherFilter.value !== '') {
    if (publisherFilter.value === 'merchant') {
      list = list.filter(c => c.merchantId != null)
    } else {
      list = list.filter(c => c.merchantId == null)
    }
  }
  ...
  if (merchantFilter.value !== '') {
    list = list.filter(c => c.merchantId != null && Number(c.merchantId) === Number(merchantFilter.value))
  }
  ...
  if (dateRange.value && dateRange.value.length === 2) {
    ...
    list = list.filter(c => {
      const startTime = new Date(c.validStartTime || 0).getTime()
      const endTime = new Date(c.validEndTime || 0).getTime()
...
const batchToggleStatus = async () => {
  ...
  const newStatus = selectedCoupons.value[0].status === 1 ? 0 : 1
  const action = newStatus === 1 ? '上架' : '下架'
  
  try {
    await ElMessageBox.confirm(
      `确认要批量${action}这 ${selectedCoupons.value.length} 张优惠券吗？`,
      '批量操作确认',
      { type: 'warning' }
    )
    
    const promises = selectedCoupons.value.map(coupon =>
      api.post(`/admin/coupon/status/${coupon.id}`, { status: newStatus })
    )
...
const batchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      `确认要删除这 ${selectedCoupons.value.length} 张优惠券吗？此操作不可恢复！`,
      '批量删除确认',
      { type: 'warning' }
    )
    
    const promises = selectedCoupons.value.map(coupon =>
      api.delete(`/admin/coupon/delete/${coupon.id}`)
    )
    await Promise.all(promises)
...
const loadCoupons = async () => {
  try {
    const res = await api.get('/admin/coupon/list')
    if (res.code === 200) {
      coupons.value = res.data || []
      console.log('加载优惠券成功，数量：', coupons.value.length)
    } else {
      console.error('加载优惠券失败，返回码：', res.code, res.message)
      ElMessage.error(res.message || '加载优惠券失败')
    }
  } catch (error) {
    console.error('加载优惠券失败', error)
    ElMessage.error(error.response?.data?.message || '加载优惠券失败')
  }
}
...
const loadMerchants = async () => {
  try {
    // 只加载审核通过的商家（status === 1）
    const res = await api.get('/admin/merchant/list', { params: { status: 1 } })
    if (res.code === 200) {
      merchants.value = (res.data || []).filter(m => m.status === 1)
      filteredMerchantList.value = merchants.value
    }
  } catch (error) {
...
const filterMerchants = (query) => {
  merchantSearchKeyword.value = query
  if (!query) {
    filteredMerchantList.value = merchants.value
    return
  }
  const keyword = query.toLowerCase()
  filteredMerchantList.value = merchants.value.filter(merchant => {
    const shopName = (merchant.shopName || '').toLowerCase()
    const username = (merchant.username || '').toLowerCase()
    return shopName.includes(keyword) || username.includes(keyword)
  })
}

const getMerchantLabel = (merchant) => {
  if (merchant.shopName) {
    return merchant.username !== merchant.shopName 
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (editingCoupon.value) {
        res = await api.put('/admin/coupon/update', form.value)
        ElMessage.success('更新成功')
      } else {
        res = await api.post('/admin/coupon/add', form.value)
        ElMessage.success('添加成功')
        console.log('添加优惠券成功，返回数据：', res)
      }
...
const toggleStatus = async (coupon) => {
  ...
  try {
    ...
    await api.post(`/admin/coupon/status/${coupon.id}`, { status: newStatus })
